...
class TimesheetController {
    async submitTimesheet(req: Request, res: Response): Promise<void> {
        try {
            const { user_id, pd_id, task_description, hours, minutes, task_status } = req.body;

            if (!user_id || !pd_id || !task_description || hours === undefined || minutes === undefined || task_status === undefined) {
                res.status(400).json({ error: 'Missing required fields' });
                return;
            }

            const query = `
                INSERT INTO trans_timesheet (user_id, pd_id, task_description, hours, minutes, task_status, is_deleted, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, 0, NOW(), NOW())`;

            db.query(query, [user_id, pd_id, task_description, hours, minutes, task_status]);
   ...
   async getUserTimesheets(req: Request, res: Response): Promise<void> {
    try {
        ...
        const query = `
            ...
            LEFT JOIN master_customer c ON m.customer_id = c.customer_id
            WHERE t.is_deleted = 0 AND t.user_id = ?
            ORDER BY t.timesheet_id DESC`; // Order by latest first
        ...
        db.query(query, [userId], (err, results) => {
            ...
            res.status(200).json(results); // Return timesheet data
        });
    } catch (error) {
   ...
   async softDeleteTimesheet(req: Request, res: Response): Promise<void> {
    try {
      ...
      const updateQuery = `UPDATE timesheets SET is_deleted = 1 WHERE timesheet_id = ?`;
